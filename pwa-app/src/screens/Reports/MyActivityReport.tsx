...
import {checkallordersinordermaster} from '../../database/SqlDatabase';
...
const MyActivityReport: React.FC = () => {
  ...
  const {Party, date, entity_id} = (location.state as any) || {};
  ...
  const renderActivityItem = (item: ActivityItem, index: number) => {
    const quantity = parseFloat(item.quantity_one || '0') + parseFloat(item.quantity_two || '0');
